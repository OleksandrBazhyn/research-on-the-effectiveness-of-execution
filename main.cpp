...
double myAlgorithm(int K, std::vector<int>& A, std::vector<int>& B) {
    ...
    for (int i = 0; i < K; i++) {
        ...
        for (int j = 0; j < num_threads && i < K; j++, i++) {
            threads.emplace_back([i, &result, &subArraysA, &subArraysB]() {
                result = ScalarProduction(subArraysA[i], subArraysB[i]);
                });
        }
...
int main()
{
    ...
    std::vector<double> times = { time1, time2, time3, time4, time5, time6 };
    printMinTime(times);
